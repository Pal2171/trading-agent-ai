...
class HyperLiquidTrader:
    ...
    def set_leverage_for_symbol(self, symbol: str, leverage: int, is_cross: bool = True) -> Dict[str, Any]:
        ...
        try:
            print(f"[INFO] Impostando leva {leverage}x per {symbol} ({'cross' if is_cross else 'isolated'} margin)")
            ...
            if result.get('status') == 'ok':
                print(f"[OK] Leva impostata con successo a {leverage}x per {symbol}")
            else:
    ...
    def execute_signal(self, order_json: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if leverage_result.get('status') != 'ok':
            print(f"[WARNING] Attenzione: impostazione leva potrebbe aver avuto problemi: {leverage_result}")
        ...
        print(f"[INFO] Leva attuale per {symbol}: {current_leverage_info}")
        ...
        if leverage > max_leverage:
            print(f"[WARNING] Leva richiesta ({leverage}) supera il massimo per {symbol} ({max_leverage})")
        ...
        if size_decimal < min_size:
            print(f"[WARNING] Size calcolata ({size_decimal}) < minima richiesta ({min_size})")
    ...
    def debug_symbol_limits(self, symbol: str = None):
        ...
        print("\n[INFO] LIMITI TRADING HYPERLIQUID")
        print("-" * 60)
