...
def get_recent_bot_operations(limit: int = 50) -> List[Dict[str, Any]]:
    """Restituisce le ultime N operazioni del bot con dettagli completi."""
    ...
    with get_connection() as conn:
        with conn.cursor() as cur:
            ...
            operations = []
            for row in rows:
                created_at, raw_payload = row
                op = {
                    'created_at': created_at.strftime('%Y-%m-%d %H:%M') if created_at else 'N/A',
                    'operation': raw_payload.get('operation', 'N/A'),
                    'symbol': raw_payload.get('symbol', 'N/A'),
                    'direction': raw_payload.get('direction', 'N/A'),
                    'leverage': raw_payload.get('leverage', 0),
                    'reason': raw_payload.get('reason', 'N/A'),
...
if __name__ == "__main__":
    ...
    # )
    # print(f"[db_utils] Operazione inserita con id={op_id}")
>>>>>>> 3a3b84d362410c395431f975158435c7efcaf0f0
